...
include('conn.php');
$record_count=$conn->query("SELECT * FROM posts where category_id='9'");
$per_page=2;
...
$prev = $page - 1;
$next = $page + 1;
$query=$conn->prepare("SELECT post_id, posted, title, LEFT  (body,100) as  body , category from posts inner join category ON category.category_id=posts.category_id where posts.category_id='9' ORDER BY post_id DESC limit $start, $per_page");
$query->execute();
	...
	<p>Time Posted :<?php  echo $posted ?></p>
	 ...
	 <?php
	 	...
	 	if ($page < $pages) {
	 		...
	 		echo "<a href='home.php?p=$next' style=' margin-right:600px; '>Next</a>";
	 	}
	 ?>
    </div>
</div>
