...
include('conn.php');
$record_count=$conn->query("SELECT body,category_id,posted,post_id,title,user_id FROM posts where category_id='1' ");
$per_page=2;
...
$prev = $page - 1;
$next = $page + 1;
$query=$conn->prepare("SELECT post_id,posted, title, LEFT  (body,100) as  body , category from posts  inner join category ON category.category_id=posts.category_id where posts.category_id='1' ORDER BY post_id DESC limit $start, $per_page");
$query->execute();
...
<div class="header">
      ...
      <h4 style="padding-left: 35px;">GENERAL</h4>
	<?php
       while($query->fetch()):
